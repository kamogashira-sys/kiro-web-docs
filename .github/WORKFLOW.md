...
**スナップショットが必要な検証だけを実行する**ターゲットもあります。
こちらは**スナップショットが無ければ失敗**します（スキップを成功にしません）。

```bash
make capture-kiro-web-snapshot        # 公式から取得（既定で当日の日付ディレクトリ）
make verify-kiro-web-with-snapshot    # 網羅性・件数の公式照合・出典日照合
```

> **CI（`verify-with-snapshot` ジョブ）はこの 2 つを毎回実行します**（2026-09-13 から）。
> それまで CI は snapshot を持たずスキップしていたため、
> **網羅性・件数の公式照合・出典日照合を CI では守れていませんでした**。
> 取得は約 40 秒・49 リクエストで、`--retries 3 --delay 0.5` を付けています。
> **取得したスナップショットはコミットしません**（kiro.dev の複製であり再配布しない）。
